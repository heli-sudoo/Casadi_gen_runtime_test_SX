...
#ifdef MATLAB_MEX_FILE
void mex_Imp_B(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  casadi_int i;
  casadi_real w[171];
  casadi_int *iw = 0;
  const casadi_real* arg[2] = {0};
  casadi_real* res[2] = {0};
  if (argc>2) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"Imp_B\" failed. Too many input arguments (%d, max 2)", argc);
  if (resc>2) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"Imp_B\" failed. Too many output arguments (%d, max 2)", resc);
  ...
  i = Imp_B(arg, res, iw, w+34, 0);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"Imp_B\" failed.");
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    mex_Imp_B(resc, resv, argc, argv);
    return;
  } else if (strcmp(buf, "Imp_B")==0) {
    mex_Imp_B(resc, resv, argc-1, argv+1);
    return;
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'Imp_B'");
}
#endif
#ifdef __cplusplus
} /* extern "C" */
