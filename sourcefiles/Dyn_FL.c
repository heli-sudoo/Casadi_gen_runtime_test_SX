...
#ifdef MATLAB_MEX_FILE
void mex_Dyn_FL(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  casadi_int i;
  casadi_real w[127];
  casadi_int *iw = 0;
  const casadi_real* arg[2] = {0};
  casadi_real* res[2] = {0};
  if (argc>2) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"Dyn_FL\" failed. Too many input arguments (%d, max 2)", argc);
  if (resc>2) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"Dyn_FL\" failed. Too many output arguments (%d, max 2)", resc);
  ...
  i = Dyn_FL(arg, res, iw, w+34, 0);
  if (i) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"Dyn_FL\" failed.");
...
void mexFunction(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (!buf_ok) {
    mex_Dyn_FL(resc, resv, argc, argv);
    return;
  } else if (strcmp(buf, "Dyn_FL")==0) {
    mex_Dyn_FL(resc, resv, argc-1, argv+1);
    return;
  }
  mexErrMsgTxt("First input should be a command string. Possible values: 'Dyn_FL'");
}
#endif
#ifdef __cplusplus
} /* extern "C" */
